/**
 * ln_cs.c,
 * coroutine switching, sync, concurrency 
 * mechanism implement by ucontext.
 *
 * lxr, 2020.01 */
...
_co_start(ci_s *ci)
{
    ...
    ci->state    = RUNNABLE;
    ctx->uc_link = &BCCTX(ci);
    ctx->uc_stack.ss_sp   = ci->stack;
    ctx->uc_stack.ss_size = ci->cc->ss;
    arg = ci->arg;
    /**
...
_co_switch(ci_s *ci)
{
    ...
    ci->state = RUNNABLE;
    ret = swapcontext(&BCCTX(ci), &ci->cctx);
    IF_EXPS_THEN_RETURN(ret, errno);
...
cs_yield(ci_s *ci)
{
    ...
    ci->state = SUSPENDING;
    ret = swapcontext(&ci->cctx, &BCCTX(ci));
    IF_EXPS_THEN_RETURN(ret, errno); 
